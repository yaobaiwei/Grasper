...
// ====== Dir Check ======
// returns -1 if fail, 0 if succeed
int out_dir_check(const char* outdir, bool print, bool force) {
    hdfsFS fs = get_hdfs_fs();
...
// ====== Dir Check ======
// returns -1 if fail, 0 if succeed
int dir_check(const char* indir, const char* outdir, bool print, bool force) {
    hdfsFS fs = get_hdfs_fs();
...
// returns -1 if fail, 0 if succeed
int dir_check(vector<string>& indirs, const char* outdir, bool print, bool force) {
    hdfsFS fs = get_hdfs_fs();
...
// returns -1 if fail, 0 if succeed
int dir_check(const char* indir, vector<string>& outdirs, bool print, bool force) {
    hdfsFS fs = get_hdfs_fs();
    if (hdfsExists(fs, indir) != 0) {
        if (print)
            fprintf(stderr, "Input path \"%s\" does not exist!\n", indir);
...
// returns -1 if fail, 0 if succeed
int dir_check(const char* outdir, bool force) {
    hdfsFS fs = get_hdfs_fs();
    if (hdfsExists(fs, outdir) == 0) {
        if (force) {
            if (hdfs_delete(fs, outdir) == -1) {
...
// returns -1 if fail, 0 if succeed
int dir_check(const char* indir) {
    hdfsFS fs = get_hdfs_fs();
    if (hdfsExists(fs, indir) != 0) {
        fprintf(stderr, "Input path \"%s\" does not exist!\n", indir);
        hdfsDisconnect(fs);
...
// force put, overwrites target
void putf(const char* localpath, const char* hdfspath) {
...
// remember to "delete[] assignment" after used
